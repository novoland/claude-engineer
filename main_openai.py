...
async def get_format_choice():
    result = await radiolist_dialog(
        title="Choose format",
        text="Select the format for saving the chat:",
        values=[
            ("markdown", "Markdown"),
            ("json", "JSON")
        ]
...
MAINMODEL = "deepseek-chat"  # Maintains conversation history and file contents

# Models that don't maintain context (memory is reset after each call)
TOOLCHECKERMODEL = "deepseek-chat"
CODEEDITORMODEL = "deepseek-chat"
CODEEXECUTIONMODEL = "deepseek-chat"
    ...
    for tool_use in tool_uses:
        ...
        try:
            ...
            for choice in tool_response.choices:
                if choice.message.content:
                    tool_checker_response += choice.message.content
            console.print(Panel(Markdown(tool_checker_response), title="Assistant's Response to Tool Result",  title_align="left", border_style="blue", expand=False))
            assistant_response += "\n\n" + tool_checker_response
        except OpenAIError as e:
